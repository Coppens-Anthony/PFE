...
Joana a la possibilité de compléter son stock en y renseignant les produits qu’elle utilise. Elle va donc pour chacun d’eux, renseigner le nombre idéal pour sa semaine. À la fin de chaque journée, elle indiquera la quantité de produit utilisée dans la journée. Au bout de chaque semaine, elle aura un petit récapitulatif des quantités qu’elle devra acheter afin de revenir au nombre idéal de son stock.

Elle pourra évidemment modifier ce nombre idéal en fonction de son historique.

🧩 Scénario 3 - Bilan de fin de mois : 
...
Elle pourra donc à chaque fin de mois consulter son bilan qui reprendra plusieurs informations utiles pour elle. Elle aura son nombre de client total sur le mois, les revenus qu’elle a réalisé ainsi que les potentiels pourboires reçus. 
...
🧩 Scénario 4 - Informations sur un client :
...
Le mois prochain, la cliente en question reprend rendez-vous. Joana voit alors, dans son planning, le rendez-vous de cette cliente avec l’information personnelle afin de préparer au mieux la séance.
...
Elle tombe alors sur le site de Joana et le consulte. Elle trouve l’endroit très professionnel et accueillant. Elle consulte ensuite les différentes propositions de coiffure et leur tarif. Elle trouve le tout très raisonnable. Elle décide alors de remplir le formulaire pour prendre un rendez-vous. Elle rentre ses informations et arrive au niveau de la coupe. Elle constate la durée moyenne de chacune. Une fois une coupe sélectionnée, les plages horaires correspondantes à la durée de celle-ci lui sont proposées et elle n’a plus qu’à en sélectionner une. 

Joana reçoit alors l’information d’une nouvelle prise de rendez-vous.

🧩 **Scénario 2 - Reprise de rendez-vous :**

Joana vient de finir la coupe de Sabrine et celle-ci demande à prendre rendez-vous pour le mois prochain. Joana accède alors à son interface d’administratrice et ajoute elle-même le nouveau rendez-vous qu’elle convînt avec Sabrine. Elle ajoutera donc la date ainsi que la coupe. Elle indiquera également les informations de Sabrine en ajoutant son adresse mail (si elle est déjà dans la base de données). Mais si Sabrine prend également rendez-vous pour son fils Mathieu qui n’est encore jamais venu et donc n’est pas dans la base de données, Joana pourra l’y ajouter avec les informations transmises via Sabrine.
